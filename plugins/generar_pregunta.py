...
def generarPregunta():
    ...
    if long > 0:
        ...
        while (True):
            ...
            contador+=1    
        file.close()
        preguntasAElegir = {'0':"Dime la traducción al Inglés de ", '1':"Dime el pasado de ", '2':"Dime el participio de "}
        preguntaElegida = preguntasAElegir[str(pregunta)]
        preguntaCompleta = preguntaElegida + verboElegido
        return 'set_slot preguntas ["{0}","{1}","{2}"]'.format(pregunta, verboElegido, preguntaCompleta)
    else:
